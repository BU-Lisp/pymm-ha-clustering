...
class DBScan(object):
    def __init__(self, epsilon: float, pymmShelf: pymm.shelf, min_points: int = 5,
        ...
        #pymmShelf.tmp_epsi = float(epsilon)
        pymmShelf.tmp_minp = int(min_points)
        pymmShelf.tmp_num_clusters = int(0)
        #self.epsilon = pymmShelf.tmp_epsi
        self.min_points = pymmShelf.tmp_minp
        self.assignemtns: pymm.ndarry=None
        self.num_clusters = pymmShelf.tmp_num_clusters
    ...
    def train(self, X: np.ndarray,
        ...
        #assignments = np.full((X.shape[0], 1), NO_CLUSTER_CLUSTER_IDX, dtype=int)
